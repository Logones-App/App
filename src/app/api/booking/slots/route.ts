...
// Fonction pour extraire et valider les paramètres
function extractAndValidateParams(request: NextRequest) {
  ...
  // Vérifier que les paramètres ne sont pas null après validation
  if (!establishmentId || !date) {
    console.error("❌ Paramètres manquants après validation");
    return { isValid: false, errors: ["Paramètres manquants"] };
  }

  return { isValid: true, establishmentId, date };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!params.isValid) {
      return NextResponse.json({ error: "Paramètres invalides", details: params.errors }, { status: 400 });
    }

    // Récupérer les créneaux
    const result = await getAvailableSlots(params.establishmentId!, params.date!);
