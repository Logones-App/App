    ...
    <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Êtes-vous sûr de vouloir supprimer l&apos;image &quot;{image.image_name ?? "Image sans nom"}&quot; ? Cette
            action est irréversible et supprimera définitivement l&apos;image.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
        ...
        <Card className="group cursor-pointer transition-all hover:shadow-md">
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              {/* Image */}
              <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded">{renderImageContent()}</div>
              ...
              {isEditable && (
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" onClick={() => onClick?.(image)}>
                    <ExternalLink className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => onEdit?.(image)}>
                    <Edit className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={handleTogglePublic}>
                    {image.is_public ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </Button>
                  <Button variant="ghost" size="sm" onClick={handleToggleFeatured}>
                    {image.is_featured ? <StarOff className="h-4 w-4" /> : <Star className="h-4 w-4" />}
                  </Button>
