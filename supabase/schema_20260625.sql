...
DECLARE
  ...
  v_sig_b64url    TEXT;
BEGIN
  -- 1. Clé de signature active (obligatoire)
  ...
  -- 2+3. Prochain event_id + signature précédente (verrou anti race-condition)
  SELECT event_id, signature_base64url
  INTO   v_event_id, v_prev_sig
  ...
  v_event_id := COALESCE(v_event_id + 1, 1);

  -- 4. Label
  v_label := 'Changement données assujetti : ' || p_changed_fields;

  -- 5. Timestamp AAAAMMJJHHMMSS (UTC)
  ...
  -- 7. HMAC-SHA256 → Base64 URL (sans padding, + → -, / → _)
  ...
  -- 8. Insertion JET 410 (purgeable = false, R19)
