...
CREATE OR REPLACE FUNCTION "public"."fn_convert"("p_value" numeric, "p_from" "text", "p_to" "text") RETURNS numeric
    LANGUAGE "sql" IMMUTABLE
    AS $$
  select case
    when p_from is null or p_to is null           then p_value
...
ALTER FUNCTION "public"."fn_convert"("p_value" numeric, "p_from" "text", "p_to" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."fn_flag_factor_mismatch"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
...
begin
  ...
  select order_unit, conversion_factor, packaging
    into v_order_unit, v_factor, v_packaging
    from public.supplier_references where id = new.supplier_reference_id;
  if v_packaging is not null then       -- colisage compté → pas de contrôle dimensionnel
    return new;
  end if;
  select unit into v_stock_unit from public.product_stocks where id = new.product_stock_id;
...
ALTER FUNCTION "public"."fn_flag_factor_mismatch"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_flag_ref_factor"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
declare v_stock_unit text; v_expected numeric;
begin
  -- Conditionnement compté (boîte, sac, plaquette…) : le facteur = nb d'unités par colis,
  -- PAS une conversion dimensionnelle → rien à valider.
  ...
  select portion_unit into v_stock_unit from public.products where id = new.product_id;
  ...
  new.needs_review := (v_expected is not null and abs(v_expected - new.conversion_factor) > 1e-6)
                      or (v_stock_unit is null);   -- SET déterministe
  return new;
end $$;


ALTER FUNCTION "public"."fn_flag_ref_factor"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE v_changed text[] := array[]::text[];
BEGIN
  IF new.name        IS DISTINCT FROM old.name        THEN v_changed := array_append(v_changed, 'nom'); END IF;
  IF new.address     IS DISTINCT FROM old.address     THEN v_changed := array_append(v_changed, 'adresse'); END IF;
  IF new.postal_code IS DISTINCT FROM old.postal_code THEN v_changed := array_append(v_changed, 'code_postal'); END IF;
  IF new.city        IS DISTINCT FROM old.city        THEN v_changed := array_append(v_changed, 'ville'); END IF;
  IF new.country     IS DISTINCT FROM old.country     THEN v_changed := array_append(v_changed, 'pays'); END IF;
  IF new.siret       IS DISTINCT FROM old.siret       THEN v_changed := array_append(v_changed, 'siret'); END IF;
  IF new.no_tva      IS DISTINCT FROM old.no_tva      THEN v_changed := array_append(v_changed, 'no_tva'); END IF;
  IF new.code_naf    IS DISTINCT FROM old.code_naf    THEN v_changed := array_append(v_changed, 'code_naf'); END IF;
  IF array_length(v_changed, 1) IS NULL THEN RETURN new; END IF;
  PERFORM public.nf525_jet_410_saas(new.id, new.organization_id, array_to_string(v_changed, ', '));
  RETURN new;
END; $$;


ALTER FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"() OWNER TO "postgres";
...
BEGIN
  -- Prix owned SaaS pour les mouvements issus d'un doc_import (OCR + réception manuelle SaaS) :
  -- le SaaS a déjà posé le snapshot → on ne le refait pas. Corrige aussi le double OCR latent.
  IF NEW.reference_type = 'doc_import' THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF v_signing_key IS NULL THEN RETURN; END IF;   -- 130 = Journal Technique, pas de blocage
  SELECT COALESCE(MAX(event_id),0)+1 INTO v_event_id FROM public.nf525_jet WHERE establishment_id = p_establishment_id;
  SELECT signature_base64url INTO v_prev_sig FROM public.nf525_jet
    WHERE establishment_id = p_establishment_id AND device_id IS NULL ORDER BY event_id DESC LIMIT 1;
  -- R19 §4.1 : séparateur canonique = "," → sanitiser le texte libre du label ("," et "|" → espace)
  v_label := 'Modification droits employé : ' || replace(replace(p_label, ',', ' '), '|', ' ');
  v_ts := to_char(NOW() AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISS');
  v_chain := v_event_id::TEXT || ',130' || ',' || v_label || ',' || v_ts || ', ' || ', '
    || ',' || CASE WHEN v_prev_sig IS NOT NULL THEN '1' ELSE '0' END || ',' || COALESCE(v_prev_sig,' ');
  ...
  VALUES (gen_random_uuid(), p_establishment_id, p_organization_id, v_event_id, 130,
...
BEGIN
  ...
  IF v_signing_key IS NULL THEN
    RAISE EXCEPTION 'nf525_jet_180_saas: aucune clé de signature active pour l''établissement %', p_establishment_id;
  END IF;
  SELECT COALESCE(MAX(event_id),0)+1 INTO v_event_id FROM public.nf525_jet WHERE establishment_id = p_establishment_id;
  SELECT signature_base64url INTO v_prev_sig FROM public.nf525_jet
    WHERE establishment_id = p_establishment_id AND device_id IS NULL ORDER BY event_id DESC LIMIT 1;
  v_label := 'Génération export écritures comptables : ' || replace(replace(p_label, ',', ' '), '|', ' ');
  v_ts := to_char(NOW() AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISS');
...
BEGIN
  SELECT signing_key_base64 INTO v_signing_key FROM public.nf525_signing_keys
    WHERE establishment_id = p_establishment_id AND valid_to IS NULL LIMIT 1 FOR UPDATE;
  IF v_signing_key IS NULL THEN
    RAISE EXCEPTION 'nf525_jet_290_saas: aucune clé de signature active pour l''établissement %', p_establishment_id;
  END IF;
  SELECT COALESCE(MAX(event_id),0)+1 INTO v_event_id FROM public.nf525_jet WHERE establishment_id = p_establishment_id;
  SELECT signature_base64url INTO v_prev_sig FROM public.nf525_jet
    WHERE establishment_id = p_establishment_id AND device_id IS NULL ORDER BY event_id DESC LIMIT 1;
  v_label := 'Échange expert-comptable (export/Z) : ' || replace(replace(p_label, ',', ' '), '|', ' ');
  v_ts := to_char(NOW() AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISS');
...
BEGIN
  SELECT signing_key_base64 INTO v_signing_key FROM public.nf525_signing_keys
    WHERE establishment_id = p_establishment_id AND valid_to IS NULL LIMIT 1 FOR UPDATE;
  IF v_signing_key IS NULL THEN
    RAISE EXCEPTION 'nf525_jet_410_saas: aucune clé de signature active pour l''établissement %', p_establishment_id;
  END IF;
  SELECT COALESCE(MAX(event_id),0)+1 INTO v_event_id FROM public.nf525_jet WHERE establishment_id = p_establishment_id;
  SELECT signature_base64url INTO v_prev_sig FROM public.nf525_jet
    WHERE establishment_id = p_establishment_id AND device_id IS NULL ORDER BY event_id DESC LIMIT 1;
  -- p_changed_fields = "siret, no_tva" (virgules) → sanitiser AVANT la chaîne signée
  v_label := 'Changement données assujetti : ' || replace(replace(p_changed_fields, ',', ' '), '|', ' ');
  v_ts := to_char(NOW() AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISS');
  v_chain := v_event_id::TEXT || ',410' || ',' || v_label || ',' || v_ts || ', ' || ', '
    || ',' || CASE WHEN v_prev_sig IS NOT NULL THEN '1' ELSE '0' END || ',' || COALESCE(v_prev_sig,' ');
  v_sig_b64 := encode(hmac(v_chain::bytea, decode(v_signing_key,'base64'),'sha256'),'base64');
  v_sig_b64url := replace(replace(replace(v_sig_b64,'+','-'),'/','_'),'=','');
  INSERT INTO public.nf525_jet (id, establishment_id, organization_id, event_id, code_event,
    label, event_at, operator_code, device_id, report_previous_signature,
    previous_signature_base64url, signature_base64url, hash_chain_input, purgeable, created_at)
  VALUES (gen_random_uuid(), p_establishment_id, p_organization_id, v_event_id, 410,
    v_label, NOW(), NULL, NULL, NULL, v_prev_sig, v_sig_b64url, v_chain, FALSE, NOW());
END; $$;
...
CREATE OR REPLACE FUNCTION "public"."purge_device_sessions_on_device_move"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  if new.organization_id is distinct from old.organization_id
    ...
    delete from public.device_sessions where device_id = new.id;
  end if;
  return new;
end;
$$;
...
ALTER FUNCTION "public"."purge_device_sessions_on_device_move"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."doc_import_lines" (
    ...
    CONSTRAINT "doc_import_lines_movement_type_check" CHECK ((("movement_type" IS NULL) OR ("movement_type" = ANY (ARRAY['purchase'::"text", 'adjustment'::"text"]))))
);
...
COMMENT ON COLUMN "public"."doc_import_lines"."movement_type" IS 'Type de mouvement pour l''ingestion POS : NULL ou ''purchase'' = réception (quantite ≥ 0) ; ''adjustment'' = correction (quantite signée +/-, pas de snapshot prix). Voir SPEC_CURRENT_STOCK_OWNERSHIP.md.';
...
CREATE TABLE IF NOT EXISTS "public"."establishments" (
    ...
    "code_naf" "text",
    "public_menu_locales" "text"[] DEFAULT '{fr}'::"text"[] NOT NULL,
    "stock_owner" "text" DEFAULT 'pos'::"text" NOT NULL,
    CONSTRAINT "establishments_stock_owner_check" CHECK (("stock_owner" = ANY (ARRAY['pos'::"text", 'saas'::"text"])))
);
...
COMMENT ON COLUMN "public"."establishments"."stock_owner" IS 'Propriétaire du compteur de stock (current_stock). ''pos'' (défaut) = le POS est seul applicateur, le SaaS n''écrit jamais en absolu (deltas via doc_imports). ''saas'' = établissement SANS caisse, écriture directe SaaS. Invariant : ''saas'' <=> aucune caisse. Voir SPEC_CURRENT_STOCK_OWNERSHIP.md.';
...
CREATE TABLE IF NOT EXISTS "public"."haccp_checklist_runs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "template_id" "uuid",
...
ALTER TABLE "public"."haccp_checklist_runs" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_checklist_runs" IS 'Exécutions de checklist (checks JSONB).';



CREATE TABLE IF NOT EXISTS "public"."haccp_checklist_templates" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "title" "text" NOT NULL,
    ...
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
    "frequency" "text" DEFAULT 'quotidien'::"text" NOT NULL,
    CONSTRAINT "haccp_checklist_templates_frequency_check" CHECK (("frequency" = ANY (ARRAY['biquotidien'::"text", 'quotidien'::"text", 'hebdomadaire'::"text", 'mensuel'::"text", 'ponctuel'::"text"])))
);


ALTER TABLE "public"."haccp_checklist_templates" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_checklist_templates" IS 'Modèles de checklist (items = tableau JSON de libellés) — config.';



COMMENT ON COLUMN "public"."haccp_checklist_templates"."frequency" IS 'Cadence structurée — pilote le tableau de bord. frequency_label reste l''affichage libre.';



CREATE TABLE IF NOT EXISTS "public"."haccp_cleaning_surfaces" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "zone_id" "uuid",
    ...
    "product" "text",
    "responsible" "text",
    CONSTRAINT "haccp_cleaning_surfaces_frequency_check" CHECK (("frequency" = ANY (ARRAY['quotidien'::"text", 'hebdomadaire'::"text", 'mensuel'::"text"])))
);


ALTER TABLE "public"."haccp_cleaning_surfaces" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_cleaning_surfaces" IS 'Surfaces à nettoyer, rattachées à une zone, avec une fréquence (quotidien/hebdomadaire/mensuel).';



COMMENT ON COLUMN "public"."haccp_cleaning_surfaces"."product" IS 'Produit d''entretien recommandé.';



CREATE TABLE IF NOT EXISTS "public"."haccp_cleaning_validations" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."haccp_cleaning_validations" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_cleaning_validations" IS 'Validations de nettoyage par surface (append). photo_path = Storage (brique #8, NULL en attendant).';



CREATE TABLE IF NOT EXISTS "public"."haccp_cooling_records" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "product_label" "text" NOT NULL,
    "started_at" timestamp with time zone NOT NULL,
    "start_temp_c" numeric NOT NULL,
    "ended_at" timestamp with time zone NOT NULL,
    "end_temp_c" numeric NOT NULL,
    "recorded_by" "uuid",
    "recorded_by_label" "text",
...
ALTER TABLE "public"."haccp_cooling_records" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_cooling_records" IS 'Refroidissements rapides (verdict ≤10°C/≤2h).';
...
CREATE TABLE IF NOT EXISTS "public"."haccp_documents" (
    ...
    "establishment_id" "uuid" NOT NULL,
    "title" "text" NOT NULL,
    "doc_type" "text" DEFAULT 'procedure'::"text" NOT NULL,
    "version" "text",
    "url" "text",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
    CONSTRAINT "haccp_documents_doc_type_check" CHECK (("doc_type" = ANY (ARRAY['plan'::"text", 'procedure'::"text", 'registre'::"text"])))
);
...
COMMENT ON TABLE "public"."haccp_documents" IS 'Bibliothèque documentaire HACCP (plans/procédures/registres) — config.';



CREATE TABLE IF NOT EXISTS "public"."haccp_label_types" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
...
ALTER TABLE "public"."haccp_label_types" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."haccp_labels" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "product_label" "text" NOT NULL,
    "produced_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "use_by_at" timestamp with time zone,
    ...
    "created_by" "uuid",
    "label_type_id" "uuid",
    "date_type" "text" DEFAULT 'DLC'::"text" NOT NULL,
    "lot_number" "text",
    "copies" integer DEFAULT 1 NOT NULL,
    "storage_temp" "text",
    "product_id" "uuid",
    "barcode" "text",
    CONSTRAINT "haccp_labels_copies_check" CHECK (("copies" > 0)),
    CONSTRAINT "haccp_labels_date_type_check" CHECK (("date_type" = ANY (ARRAY['DLC'::"text", 'DDM'::"text"])))
);


ALTER TABLE "public"."haccp_labels" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_labels" IS 'Étiquettes DLC + allergènes (14 INCO).';



CREATE TABLE IF NOT EXISTS "public"."haccp_non_conformities" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "reference" "text",
    ...
    "reception_id" "uuid",
    "zone_id" "uuid",
    "severity" "text" DEFAULT 'majeure'::"text" NOT NULL,
    ...
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
    CONSTRAINT "haccp_non_conformities_category_check" CHECK (("category" = ANY (ARRAY['temperature'::"text", 'reception'::"text", 'nettoyage'::"text", 'huile'::"text", 'cuisson'::"text", 'tracabilite'::"text", 'hygiene_personnel'::"text", 'nuisibles'::"text", 'equipement'::"text", 'autre'::"text"]))),
    CONSTRAINT "haccp_non_conformities_severity_check" CHECK (("severity" = ANY (ARRAY['mineure'::"text", 'majeure'::"text", 'critique'::"text"]))),
    CONSTRAINT "haccp_non_conformities_source_type_check" CHECK (("source_type" = ANY (ARRAY['reception'::"text", 'temperature_reading'::"text", 'oil_test'::"text", 'cleaning_validation'::"text", 'product_temp_control'::"text", 'checklist_run'::"text", 'production_lot'::"text", 'autre'::"text"]))),
    CONSTRAINT "haccp_non_conformities_status_check" CHECK (("status" = ANY (ARRAY['ouvert'::"text", 'en_cours'::"text", 'cloture'::"text"])))
);


ALTER TABLE "public"."haccp_non_conformities" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_non_conformities" IS 'Pivot des non-conformités HACCP (toutes origines). Source de vérité SaaS, écriture mixte SaaS+mobile.';



CREATE TABLE IF NOT EXISTS "public"."haccp_oil_baths" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "label" "text" NOT NULL,
    ...
    "oil_type" "text",
    "zone_id" "uuid",
    CONSTRAINT "haccp_oil_baths_frequency_check" CHECK (("frequency" = ANY (ARRAY['biquotidien'::"text", 'quotidien'::"text", 'hebdomadaire'::"text", 'mensuel'::"text", 'ponctuel'::"text"])))
);


ALTER TABLE "public"."haccp_oil_baths" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_oil_baths" IS 'Bains de friture contrôlés (équipements) — config.';



COMMENT ON COLUMN "public"."haccp_oil_baths"."frequency" IS 'Cadence de contrôle de la polarité — pilote le tableau de bord des tâches.';



COMMENT ON COLUMN "public"."haccp_oil_baths"."zone_id" IS 'Zone HACCP du bain (optionnel).';



CREATE TABLE IF NOT EXISTS "public"."haccp_oil_tests" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "bath_id" "uuid",
    "tested_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "polarity_pct" numeric,
    "changed" boolean DEFAULT false NOT NULL,
    "recorded_by" "uuid",
    "recorded_by_label" "text",
    "conform" boolean DEFAULT true NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
...
ALTER TABLE "public"."haccp_oil_tests" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_oil_tests" IS 'Tests de polarité des bains de friture (seuil 25%).';



COMMENT ON COLUMN "public"."haccp_oil_tests"."organoleptic_ok" IS 'Contrôle organoleptique (couleur/odeur) conforme ? Saisi au test côté mobile.';



CREATE TABLE IF NOT EXISTS "public"."haccp_product_temp_controls" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    ...
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
    CONSTRAINT "haccp_product_temp_controls_control_type_check" CHECK (("control_type" = ANY (ARRAY['cuisson'::"text", 'remise_en_chauffe'::"text", 'maintien_chaud'::"text"])))
);


ALTER TABLE "public"."haccp_product_temp_controls" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_product_temp_controls" IS 'Contrôles de T° produit à cœur (cuisson/remise en chauffe/maintien chaud). Registre mobile. Refroidissement = haccp_cooling_records.';



CREATE TABLE IF NOT EXISTS "public"."haccp_production_lots" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "preparation_label" "text" NOT NULL,
    "lot_code" "text",
    "produced_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "quantity_label" "text",
    "recorded_by" "uuid",
...
ALTER TABLE "public"."haccp_production_lots" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_production_lots" IS 'Lots de production + étapes critiques (steps JSONB).';
...
CREATE TABLE IF NOT EXISTS "public"."haccp_receptions" (
    ...
    "overall_conform" boolean DEFAULT true NOT NULL,
    "reserve" "text",
    "storage_types" "text"[] DEFAULT '{}'::"text"[] NOT NULL,
    "cold_temp_c" numeric,
    "delivery_nc" "jsonb" DEFAULT '[]'::"jsonb" NOT NULL,
    "items" "jsonb" DEFAULT '[]'::"jsonb" NOT NULL,
    "doc_import_id" "uuid",
    "bl_number" "text",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "public"."haccp_receptions" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_receptions" IS 'Contrôle à réception (conformité globale + NC/items JSONB). Peut être pré-rempli par un doc_import.';



COMMENT ON COLUMN "public"."haccp_receptions"."supplier_id" IS 'Fournisseur du référentiel (suppliers). NULL = saisie libre/OCR non rapprochée (supplier_label seul).';



CREATE TABLE IF NOT EXISTS "public"."haccp_temperature_probes" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "zone_id" "uuid",
    ...
    "deleted" boolean DEFAULT false NOT NULL,
    "created_by" "uuid",
    "frequency" "text" DEFAULT 'biquotidien'::"text" NOT NULL,
    CONSTRAINT "haccp_temperature_probes_frequency_check" CHECK (("frequency" = ANY (ARRAY['biquotidien'::"text", 'quotidien'::"text", 'hebdomadaire'::"text", 'mensuel'::"text", 'ponctuel'::"text"])))
);


ALTER TABLE "public"."haccp_temperature_probes" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_temperature_probes" IS 'Équipements froid/chaud contrôlés (sondes). min_c/max_c = seuils de conformité.';



COMMENT ON COLUMN "public"."haccp_temperature_probes"."frequency" IS 'Cadence de relevé PAR ÉQUIPEMENT (défaut biquotidien = 2×/jour) — pilote le tableau de bord des tâches.';
...
ALTER TABLE "public"."haccp_temperature_readings" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_temperature_readings" IS 'Relevés de température par sonde (append). recorded_by_label = nom dénormalisé (survit à la suppression employé).';



CREATE TABLE IF NOT EXISTS "public"."haccp_trace_detailed" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "recorded_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "public"."haccp_trace_detailed" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_trace_detailed" IS 'Traçabilité détaillée (lignes lot/DLC en JSONB). reception_id = lien souple vers haccp_receptions.';



CREATE TABLE IF NOT EXISTS "public"."haccp_trace_simple" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "establishment_id" "uuid" NOT NULL,
    "recorded_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "public"."haccp_trace_simple" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_trace_simple" IS 'Traçabilité simple (photo horodatée). photo_path = Storage (brique #8).';
...
ALTER TABLE "public"."haccp_zones" OWNER TO "postgres";


COMMENT ON TABLE "public"."haccp_zones" IS 'Zones HACCP (frigo, réserve, cuisine…) — partagées par les sondes de température et le plan de nettoyage.';
...
CREATE TABLE IF NOT EXISTS "public"."printers" (
    ...
    "is_default" boolean DEFAULT false NOT NULL,
    "vendor" "text",
    CONSTRAINT "printers_devicetype_check" CHECK (("devicetype" = ANY (ARRAY['receipt'::"text", 'label'::"text"]))),
    CONSTRAINT "printers_vendor_check" CHECK (("vendor" = ANY (ARRAY['epson'::"text", 'sunmi'::"text", 'brother'::"text", 'unknown'::"text"])))
);
...
COMMENT ON COLUMN "public"."supplier_references"."conversion_factor" IS 'Nombre d''unités de stock (portion_unit du produit) contenues dans 1 unité de commande (order_unit). Ex : 1 kg = 1000 g -> 1000. Doit valoir fn_convert(1, order_unit, stock_unit) quand les unités sont convertibles.';
...
CREATE TABLE IF NOT EXISTS "public"."suppliers" (
    ...
    "ce_approval_number" "text",
    "certifications" "text"[] DEFAULT '{}'::"text"[],
    "quality_rating" smallint,
    CONSTRAINT "suppliers_quality_rating_check" CHECK ((("quality_rating" IS NULL) OR (("quality_rating" >= 1) AND ("quality_rating" <= 5))))
);
...
ALTER TABLE ONLY "public"."haccp_checklist_runs"
    ADD CONSTRAINT "haccp_checklist_runs_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_checklist_templates"
    ADD CONSTRAINT "haccp_checklist_templates_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_cleaning_surfaces"
    ADD CONSTRAINT "haccp_cleaning_surfaces_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_cleaning_validations"
    ADD CONSTRAINT "haccp_cleaning_validations_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_cooling_records"
    ADD CONSTRAINT "haccp_cooling_records_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."haccp_label_types"
    ADD CONSTRAINT "haccp_label_types_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_oil_baths"
    ADD CONSTRAINT "haccp_oil_baths_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_oil_tests"
    ADD CONSTRAINT "haccp_oil_tests_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_product_temp_controls"
    ADD CONSTRAINT "haccp_product_temp_controls_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_production_lots"
    ADD CONSTRAINT "haccp_production_lots_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_temperature_probes"
    ADD CONSTRAINT "haccp_temperature_probes_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_temperature_readings"
    ADD CONSTRAINT "haccp_temperature_readings_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_trace_detailed"
    ADD CONSTRAINT "haccp_trace_detailed_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_trace_simple"
    ADD CONSTRAINT "haccp_trace_simple_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."haccp_zones"
    ADD CONSTRAINT "haccp_zones_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_haccp_cleanval_est" ON "public"."haccp_cleaning_validations" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_cleanval_surface" ON "public"."haccp_cleaning_validations" USING "btree" ("surface_id", "validated_at" DESC);



CREATE INDEX "idx_haccp_clruns_est" ON "public"."haccp_checklist_runs" USING "btree" ("establishment_id", "run_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_cltemplates_est" ON "public"."haccp_checklist_templates" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_cooling_est" ON "public"."haccp_cooling_records" USING "btree" ("establishment_id", "started_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_documents_est" ON "public"."haccp_documents" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_labels_est" ON "public"."haccp_labels" USING "btree" ("establishment_id", "produced_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_nc_est_status" ON "public"."haccp_non_conformities" USING "btree" ("establishment_id", "status", "detected_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_nc_reception" ON "public"."haccp_non_conformities" USING "btree" ("reception_id");



CREATE INDEX "idx_haccp_nc_source" ON "public"."haccp_non_conformities" USING "btree" ("source_type", "source_id");



CREATE INDEX "idx_haccp_oilbaths_est" ON "public"."haccp_oil_baths" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_oilbaths_zone" ON "public"."haccp_oil_baths" USING "btree" ("zone_id");



CREATE INDEX "idx_haccp_oiltests_bath" ON "public"."haccp_oil_tests" USING "btree" ("bath_id", "tested_at" DESC);



CREATE INDEX "idx_haccp_oiltests_est" ON "public"."haccp_oil_tests" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_probes_est" ON "public"."haccp_temperature_probes" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_probes_zone" ON "public"."haccp_temperature_probes" USING "btree" ("zone_id");



CREATE INDEX "idx_haccp_prodlots_est" ON "public"."haccp_production_lots" USING "btree" ("establishment_id", "produced_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_prodtemp_est" ON "public"."haccp_product_temp_controls" USING "btree" ("establishment_id", "measured_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_prodtemp_lot" ON "public"."haccp_product_temp_controls" USING "btree" ("production_lot_id");



CREATE INDEX "idx_haccp_readings_est" ON "public"."haccp_temperature_readings" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_readings_probe" ON "public"."haccp_temperature_readings" USING "btree" ("probe_id", "recorded_at" DESC);



CREATE INDEX "idx_haccp_receptions_est" ON "public"."haccp_receptions" USING "btree" ("establishment_id", "received_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_receptions_supplier" ON "public"."haccp_receptions" USING "btree" ("supplier_id");



CREATE INDEX "idx_haccp_surfaces_est" ON "public"."haccp_cleaning_surfaces" USING "btree" ("establishment_id") WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_surfaces_zone" ON "public"."haccp_cleaning_surfaces" USING "btree" ("zone_id");



CREATE INDEX "idx_haccp_tracedet_est" ON "public"."haccp_trace_detailed" USING "btree" ("establishment_id", "recorded_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_tracesimple_est" ON "public"."haccp_trace_simple" USING "btree" ("establishment_id", "recorded_at" DESC) WHERE ("deleted" = false);



CREATE INDEX "idx_haccp_zones_est" ON "public"."haccp_zones" USING "btree" ("establishment_id") WHERE ("deleted" = false);
...
CREATE INDEX "idx_public_menu_sections_parent" ON "public"."public_menu_sections" USING "btree" ("parent_id");
...
CREATE INDEX "public_menu_sections_menu_id_idx" ON "public"."public_menu_sections" USING "btree" ("menu_id");
...
CREATE OR REPLACE VIEW "public"."v_stock_reconciliation" AS
 SELECT "ps"."id" AS "product_stock_id",
   ...
   FROM ("public"."product_stocks" "ps"
     LEFT JOIN "public"."stock_movements" "sm" ON ((("sm"."product_stock_id" = "ps"."id") AND (("sm"."movement_type")::"text" = ANY (ARRAY['purchase'::"text", 'production'::"text", 'adjustment'::"text"])) AND ("sm"."remaining_quantity" > (0)::numeric) AND ("sm"."deleted" = false))))
...
CREATE OR REPLACE TRIGGER "handle_updated_at_haccp_label_types" BEFORE UPDATE ON "public"."haccp_label_types" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();
...
CREATE OR REPLACE TRIGGER "handle_updated_at_product_option_group_products" BEFORE UPDATE ON "public"."product_option_group_products" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_updated_at_product_option_group_values" BEFORE UPDATE ON "public"."product_option_group_values" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();



CREATE OR REPLACE TRIGGER "handle_updated_at_product_option_groups" BEFORE UPDATE ON "public"."product_option_groups" FOR EACH ROW EXECUTE FUNCTION "public"."handle_updated_at"();
...
CREATE OR REPLACE TRIGGER "purge_device_sessions_on_device_move" AFTER UPDATE ON "public"."devices" FOR EACH ROW EXECUTE FUNCTION "public"."purge_device_sessions_on_device_move"();
...
CREATE OR REPLACE TRIGGER "trg_flag_factor_mismatch" BEFORE INSERT ON "public"."stock_movements" FOR EACH ROW EXECUTE FUNCTION "public"."fn_flag_factor_mismatch"();



CREATE OR REPLACE TRIGGER "trg_flag_ref_factor" BEFORE INSERT OR UPDATE ON "public"."supplier_references" FOR EACH ROW EXECUTE FUNCTION "public"."fn_flag_ref_factor"();
...
CREATE OR REPLACE TRIGGER "trg_nf525_jet_410_establishment" AFTER UPDATE ON "public"."establishments" FOR EACH ROW EXECUTE FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"();
...
ALTER TABLE ONLY "public"."haccp_checklist_runs"
    ADD CONSTRAINT "haccp_checklist_runs_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_checklist_runs"
    ADD CONSTRAINT "haccp_checklist_runs_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_checklist_runs"
    ADD CONSTRAINT "haccp_checklist_runs_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_checklist_runs"
    ADD CONSTRAINT "haccp_checklist_runs_template_id_fkey" FOREIGN KEY ("template_id") REFERENCES "public"."haccp_checklist_templates"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_checklist_templates"
    ADD CONSTRAINT "haccp_checklist_templates_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_checklist_templates"
    ADD CONSTRAINT "haccp_checklist_templates_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_cleaning_surfaces"
    ADD CONSTRAINT "haccp_cleaning_surfaces_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_cleaning_surfaces"
    ADD CONSTRAINT "haccp_cleaning_surfaces_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_cleaning_surfaces"
    ADD CONSTRAINT "haccp_cleaning_surfaces_zone_id_fkey" FOREIGN KEY ("zone_id") REFERENCES "public"."haccp_zones"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_cleaning_validations"
    ADD CONSTRAINT "haccp_cleaning_validations_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_cleaning_validations"
    ADD CONSTRAINT "haccp_cleaning_validations_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_cleaning_validations"
    ADD CONSTRAINT "haccp_cleaning_validations_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_cleaning_validations"
    ADD CONSTRAINT "haccp_cleaning_validations_surface_id_fkey" FOREIGN KEY ("surface_id") REFERENCES "public"."haccp_cleaning_surfaces"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."haccp_cooling_records"
    ADD CONSTRAINT "haccp_cooling_records_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_cooling_records"
    ADD CONSTRAINT "haccp_cooling_records_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_cooling_records"
    ADD CONSTRAINT "haccp_cooling_records_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_documents"
    ADD CONSTRAINT "haccp_documents_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_documents"
    ADD CONSTRAINT "haccp_documents_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_labels"
    ADD CONSTRAINT "haccp_labels_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_labels"
    ADD CONSTRAINT "haccp_labels_label_type_id_fkey" FOREIGN KEY ("label_type_id") REFERENCES "public"."haccp_label_types"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_labels"
    ADD CONSTRAINT "haccp_labels_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_labels"
    ADD CONSTRAINT "haccp_labels_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_assigned_to_fkey" FOREIGN KEY ("assigned_to") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_closed_by_fkey" FOREIGN KEY ("closed_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_reception_id_fkey" FOREIGN KEY ("reception_id") REFERENCES "public"."haccp_receptions"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_non_conformities"
    ADD CONSTRAINT "haccp_non_conformities_zone_id_fkey" FOREIGN KEY ("zone_id") REFERENCES "public"."haccp_zones"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_oil_baths"
    ADD CONSTRAINT "haccp_oil_baths_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_oil_baths"
    ADD CONSTRAINT "haccp_oil_baths_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_oil_baths"
    ADD CONSTRAINT "haccp_oil_baths_zone_id_fkey" FOREIGN KEY ("zone_id") REFERENCES "public"."haccp_zones"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_oil_tests"
    ADD CONSTRAINT "haccp_oil_tests_bath_id_fkey" FOREIGN KEY ("bath_id") REFERENCES "public"."haccp_oil_baths"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_oil_tests"
    ADD CONSTRAINT "haccp_oil_tests_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_oil_tests"
    ADD CONSTRAINT "haccp_oil_tests_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_oil_tests"
    ADD CONSTRAINT "haccp_oil_tests_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_product_temp_controls"
    ADD CONSTRAINT "haccp_product_temp_controls_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_product_temp_controls"
    ADD CONSTRAINT "haccp_product_temp_controls_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_product_temp_controls"
    ADD CONSTRAINT "haccp_product_temp_controls_production_lot_id_fkey" FOREIGN KEY ("production_lot_id") REFERENCES "public"."haccp_production_lots"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_product_temp_controls"
    ADD CONSTRAINT "haccp_product_temp_controls_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_production_lots"
    ADD CONSTRAINT "haccp_production_lots_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_production_lots"
    ADD CONSTRAINT "haccp_production_lots_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_production_lots"
    ADD CONSTRAINT "haccp_production_lots_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_doc_import_id_fkey" FOREIGN KEY ("doc_import_id") REFERENCES "public"."doc_imports"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_received_by_fkey" FOREIGN KEY ("received_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_receptions"
    ADD CONSTRAINT "haccp_receptions_supplier_id_fkey" FOREIGN KEY ("supplier_id") REFERENCES "public"."suppliers"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_temperature_probes"
    ADD CONSTRAINT "haccp_temperature_probes_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_temperature_probes"
    ADD CONSTRAINT "haccp_temperature_probes_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_temperature_probes"
    ADD CONSTRAINT "haccp_temperature_probes_zone_id_fkey" FOREIGN KEY ("zone_id") REFERENCES "public"."haccp_zones"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_temperature_readings"
    ADD CONSTRAINT "haccp_temperature_readings_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_temperature_readings"
    ADD CONSTRAINT "haccp_temperature_readings_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_temperature_readings"
    ADD CONSTRAINT "haccp_temperature_readings_probe_id_fkey" FOREIGN KEY ("probe_id") REFERENCES "public"."haccp_temperature_probes"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."haccp_temperature_readings"
    ADD CONSTRAINT "haccp_temperature_readings_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_trace_detailed"
    ADD CONSTRAINT "haccp_trace_detailed_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_trace_detailed"
    ADD CONSTRAINT "haccp_trace_detailed_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_trace_detailed"
    ADD CONSTRAINT "haccp_trace_detailed_reception_id_fkey" FOREIGN KEY ("reception_id") REFERENCES "public"."haccp_receptions"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_trace_detailed"
    ADD CONSTRAINT "haccp_trace_detailed_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_trace_simple"
    ADD CONSTRAINT "haccp_trace_simple_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_trace_simple"
    ADD CONSTRAINT "haccp_trace_simple_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");



ALTER TABLE ONLY "public"."haccp_trace_simple"
    ADD CONSTRAINT "haccp_trace_simple_recorded_by_fkey" FOREIGN KEY ("recorded_by") REFERENCES "public"."employees"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."haccp_zones"
    ADD CONSTRAINT "haccp_zones_establishment_id_fkey" FOREIGN KEY ("establishment_id") REFERENCES "public"."establishments"("id");



ALTER TABLE ONLY "public"."haccp_zones"
    ADD CONSTRAINT "haccp_zones_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id");
...
ALTER TABLE ONLY "public"."public_menu_sections"
    ADD CONSTRAINT "public_menu_sections_menu_id_fkey" FOREIGN KEY ("menu_id") REFERENCES "public"."menus"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."public_menu_sections"
    ADD CONSTRAINT "public_menu_sections_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."public_menu_sections"("id") ON DELETE CASCADE;
...
ALTER TABLE "public"."haccp_checklist_runs" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_checklist_runs_delete_universal" ON "public"."haccp_checklist_runs" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_runs_insert_universal" ON "public"."haccp_checklist_runs" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_runs_select_universal" ON "public"."haccp_checklist_runs" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_runs_update_universal" ON "public"."haccp_checklist_runs" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_checklist_templates" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_checklist_templates_delete_universal" ON "public"."haccp_checklist_templates" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_templates_insert_universal" ON "public"."haccp_checklist_templates" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_templates_select_universal" ON "public"."haccp_checklist_templates" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_checklist_templates_update_universal" ON "public"."haccp_checklist_templates" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_cleaning_surfaces" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_cleaning_surfaces_delete_universal" ON "public"."haccp_cleaning_surfaces" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_surfaces_insert_universal" ON "public"."haccp_cleaning_surfaces" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_surfaces_select_universal" ON "public"."haccp_cleaning_surfaces" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_surfaces_update_universal" ON "public"."haccp_cleaning_surfaces" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_cleaning_validations" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_cleaning_validations_delete_universal" ON "public"."haccp_cleaning_validations" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_validations_insert_universal" ON "public"."haccp_cleaning_validations" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_validations_select_universal" ON "public"."haccp_cleaning_validations" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cleaning_validations_update_universal" ON "public"."haccp_cleaning_validations" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_cooling_records" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_cooling_records_delete_universal" ON "public"."haccp_cooling_records" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cooling_records_insert_universal" ON "public"."haccp_cooling_records" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cooling_records_select_universal" ON "public"."haccp_cooling_records" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_cooling_records_update_universal" ON "public"."haccp_cooling_records" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));
...
CREATE POLICY "haccp_documents_delete_universal" ON "public"."haccp_documents" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_documents_insert_universal" ON "public"."haccp_documents" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_documents_select_universal" ON "public"."haccp_documents" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_documents_update_universal" ON "public"."haccp_documents" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_label_types" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_label_types_delete_universal" ON "public"."haccp_label_types" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_label_types_insert_universal" ON "public"."haccp_label_types" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_label_types_select_universal" ON "public"."haccp_label_types" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_label_types_update_universal" ON "public"."haccp_label_types" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_labels" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_labels_delete_universal" ON "public"."haccp_labels" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_labels_insert_universal" ON "public"."haccp_labels" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_labels_select_universal" ON "public"."haccp_labels" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_labels_update_universal" ON "public"."haccp_labels" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_non_conformities" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_non_conformities_delete_universal" ON "public"."haccp_non_conformities" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_non_conformities_insert_universal" ON "public"."haccp_non_conformities" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_non_conformities_select_universal" ON "public"."haccp_non_conformities" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_non_conformities_update_universal" ON "public"."haccp_non_conformities" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_oil_baths" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_oil_baths_delete_universal" ON "public"."haccp_oil_baths" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_baths_insert_universal" ON "public"."haccp_oil_baths" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_baths_select_universal" ON "public"."haccp_oil_baths" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_baths_update_universal" ON "public"."haccp_oil_baths" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_oil_tests" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_oil_tests_delete_universal" ON "public"."haccp_oil_tests" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_tests_insert_universal" ON "public"."haccp_oil_tests" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_tests_select_universal" ON "public"."haccp_oil_tests" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_oil_tests_update_universal" ON "public"."haccp_oil_tests" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_product_temp_controls" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_product_temp_controls_delete_universal" ON "public"."haccp_product_temp_controls" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_product_temp_controls_insert_universal" ON "public"."haccp_product_temp_controls" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_product_temp_controls_select_universal" ON "public"."haccp_product_temp_controls" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_product_temp_controls_update_universal" ON "public"."haccp_product_temp_controls" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_production_lots" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_production_lots_delete_universal" ON "public"."haccp_production_lots" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_production_lots_insert_universal" ON "public"."haccp_production_lots" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_production_lots_select_universal" ON "public"."haccp_production_lots" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_production_lots_update_universal" ON "public"."haccp_production_lots" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));
...
CREATE POLICY "haccp_receptions_delete_universal" ON "public"."haccp_receptions" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_receptions_insert_universal" ON "public"."haccp_receptions" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_receptions_select_universal" ON "public"."haccp_receptions" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_receptions_update_universal" ON "public"."haccp_receptions" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_temperature_probes" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_temperature_probes_delete_universal" ON "public"."haccp_temperature_probes" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_probes_insert_universal" ON "public"."haccp_temperature_probes" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_probes_select_universal" ON "public"."haccp_temperature_probes" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_probes_update_universal" ON "public"."haccp_temperature_probes" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));
...
CREATE POLICY "haccp_temperature_readings_delete_universal" ON "public"."haccp_temperature_readings" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_readings_insert_universal" ON "public"."haccp_temperature_readings" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_readings_select_universal" ON "public"."haccp_temperature_readings" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_temperature_readings_update_universal" ON "public"."haccp_temperature_readings" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_trace_detailed" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_trace_detailed_delete_universal" ON "public"."haccp_trace_detailed" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_detailed_insert_universal" ON "public"."haccp_trace_detailed" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_detailed_select_universal" ON "public"."haccp_trace_detailed" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_detailed_update_universal" ON "public"."haccp_trace_detailed" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_trace_simple" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_trace_simple_delete_universal" ON "public"."haccp_trace_simple" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_simple_insert_universal" ON "public"."haccp_trace_simple" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_simple_select_universal" ON "public"."haccp_trace_simple" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_trace_simple_update_universal" ON "public"."haccp_trace_simple" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



ALTER TABLE "public"."haccp_zones" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "haccp_zones_delete_universal" ON "public"."haccp_zones" FOR DELETE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_zones_insert_universal" ON "public"."haccp_zones" FOR INSERT TO "authenticated" WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_zones_select_universal" ON "public"."haccp_zones" FOR SELECT TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));



CREATE POLICY "haccp_zones_update_universal" ON "public"."haccp_zones" FOR UPDATE TO "authenticated" USING ("public"."auth_can_access_establishment"("organization_id", "establishment_id")) WITH CHECK ("public"."auth_can_access_establishment"("organization_id", "establishment_id"));
...
GRANT ALL ON FUNCTION "public"."fn_convert"("p_value" numeric, "p_from" "text", "p_to" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_convert"("p_value" numeric, "p_from" "text", "p_to" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_convert"("p_value" numeric, "p_from" "text", "p_to" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_flag_factor_mismatch"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_flag_factor_mismatch"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_flag_factor_mismatch"() TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_flag_ref_factor"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_flag_ref_factor"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_flag_ref_factor"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_nf525_jet_410_on_establishment_update"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."purge_device_sessions_on_device_move"() TO "anon";
GRANT ALL ON FUNCTION "public"."purge_device_sessions_on_device_move"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."purge_device_sessions_on_device_move"() TO "service_role";
...
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_runs" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_runs" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_runs" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_templates" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_templates" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_checklist_templates" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_surfaces" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_surfaces" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_surfaces" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_validations" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_validations" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cleaning_validations" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cooling_records" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cooling_records" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_cooling_records" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_documents" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_documents" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_documents" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_label_types" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_label_types" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_label_types" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_labels" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_labels" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_labels" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_non_conformities" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_non_conformities" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_non_conformities" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_baths" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_baths" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_baths" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_tests" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_tests" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_oil_tests" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_product_temp_controls" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_product_temp_controls" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_product_temp_controls" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_production_lots" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_production_lots" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_production_lots" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_receptions" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_receptions" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_receptions" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_probes" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_probes" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_probes" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_readings" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_readings" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_temperature_readings" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_detailed" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_detailed" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_detailed" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_simple" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_simple" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_trace_simple" TO "service_role";



GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_zones" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_zones" TO "authenticated";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,UPDATE ON TABLE "public"."haccp_zones" TO "service_role";
